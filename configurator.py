    ...
    with open("config-new.toml", "w") as outfile:
        for line in infile:
            ...
            if line.startswith("input_images"):
                input_images = crossfiledialog.choose_folder(title="Choose the directory where the camera folders are stored.")
                outfile.write(f"input_images = \"{input_images}\"\n")
            # Images output
            elif line.startswith("images_output"):
                images_output = crossfiledialog.choose_folder(title="Choose the directory where processed images should go.")
                outfile.write(f"images_output = \"{images_output}\"\n")
            # Trainee model
            elif line.startswith("trainee_model"):
                trainee_model = crossfiledialog.choose_folder(title="Choose the directory where the downloaded model should be saved.")
            ...
            # Trained model and model output
                ...
                models_output = crossfiledialog.choose_folder(title="Choose the directory where trained models should go.")
