...
def load_model():
    ...
    # Update model initialization to include time embedding
    model = snowPoleResNet50(
        pretrained=False, 
        requires_grad=False, 
        num_cameras=num_cameras,
        embedding_dim=64,  # Should match training
        time_embedding_dim=32  # Should match training
    ).to(args.device)
    
    # load the model checkpoint
    model_path = f"{args.model}/model.pth"
    # Allow SmoothL1Loss when loading the model
...
def predict(model, data, camera_mapping, eval='eval'): 
    ...
    labels =  pd.read_csv('/Users/cmbreen/code/snowpoles/models/trained_alaska_cnn_allpoles_camembedtimeembed_lr1e4/valid_samples.csv')
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(data)): 
            ...
            # Get datetime string if available, otherwise use None
            datetime_str = data.get('datetime', None)
            datetime_strings = [datetime_str] if datetime_str else None
            ...
            # Pass datetime strings to model if available
