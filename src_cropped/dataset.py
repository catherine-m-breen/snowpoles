...
class snowPoleDataset(Dataset):
    ...
    def __init__(self, samples, path, aug, crop_params=None): # split='train'):
        self.data = samples
        self.path = path
        self.resize = 224
        self.crop_params = crop_params  # New parameter for crop settings
    ...
    def apply_crop_and_transform_keypoints(self, image, keypoints):
        """
        Apply cropping to image and transform keypoints accordingly
        """
        if self.crop_params is None:
            return image, keypoints
        
        y_start, y_end, x_start, x_end = self.crop_params
        ...
        for i in range(0, len(keypoints), 2):  # Process x,y pairs
            x_coord = keypoints[i] - x_start
            y_coord = keypoints[i+1] - y_start
            
            # Check if keypoint is within crop bounds
            crop_height, crop_width = cropped_image.shape[:2]
            if 0 <= x_coord <= crop_width and 0 <= y_coord <= crop_height:
                transformed_keypoints.extend([x_coord, y_coord])
            else:
                # Handle keypoints outside crop - you might want to skip or clip
                transformed_keypoints.extend([max(0, min(x_coord, crop_width)), 
                                           max(0, min(y_coord, crop_height))])
        
        return cropped_image, np.array(transformed_keypoints, dtype='float32')
    ...
    def __getitem__(self, index):
        ...
        # get the keypoints BEFORE resizing
        keypoints = self.data.iloc[index][1:][['x1','y1','x2','y2']]
        keypoints = keypoints.clip(lower=0)
        keypoints = np.array(keypoints, dtype='float32')
        
        # Apply cropping and keypoint transformation BEFORE resizing
        image, keypoints = self.apply_crop_and_transform_keypoints(image, keypoints)
        ...
        # Scale keypoints based on crop dimensions
...
crop_settings = (50, 450, 150, 250)  # (y_start, y_end, x_start, x_end)
...
# initialize the dataset with crop parameters
train_data = snowPoleDataset(
    training_samples,
    f"{config['paths']['input_images']}",
    aug=config['training']['aug'],
    crop_params=crop_settings  # Add crop parameters
)
