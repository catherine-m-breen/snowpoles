...
def valid_keypoints_plot(image, outputs, orig_keypoints, epoch):
    ...
    # [0] just get a single datapoint from each batch
    # [-1] get the last image in the dataset
    img = image[0][-1]  ## something snow in it ## halfway throught the dataset
    ...
    for p in range(output_keypoint.shape[0]):
        if p == 0: 
            ...
            plt.plot(orig_keypoint[p, 0], orig_keypoint[p, 1], 'b.')
    plt.savefig(f"{config['paths']['models_output']}/val_epoch_{epoch}.png")
    plt.close()
